...
if (!isset($_SESSION['empid'])){// Print a customized message.
    ...
    echo ("<center>"); 
    echo ("<h1 class='pagetitle'>Employee Performance - {$selectedMonth} {$selectedYear}</h1>");
    ...
    SELECT 
        ...
        COUNT(t.TransID) AS TransactionsHandled,
        SUM(tm.Price) AS TotalRevenue
    FROM Transactions t
    JOIN TransactionMovies tm ON t.TransID = tm.TransID
    JOIN Users u ON t.EmpID = u.EmpID
    WHERE YEAR(t.RentalDate) = '$selectedYear' 
    AND MONTH(t.RentalDate) = '$monthNumber'
    GROUP BY u.EmpID, Year, Month
    ...
    echo "<table id='allTables'><tr>
        <th>Employee ID</th><th>Employee Name</th><th>Month</th><th>Year</th><th>Transactions Handled</th><th>Total Revenue</th></tr>"; 
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
        ...
        echo "<td>" . $row['Year'] . "</td>"; 
        echo "<td>" . $row['TransactionsHandled'] . "</td>"; 
        echo "<td>" . $row['TotalRevenue'] . "</td></tr>";  
    }
    echo "</table>";   
    echo("
